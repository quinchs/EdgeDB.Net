...
namespace EdgeDB.CLI
{
    ...
    {
        public static async Task<GenerationResult> ParseAndGenerateAsync(EdgeDBTcpClient client, string edgeql, string @namespace, string hash, string name)
        {
            var parseResult = await client.ParseAsync(edgeql, Cardinality.Many, IOFormat.Binary, Capabilities.All, default);

            return GenerateCSharpFromEdgeQL(@namespace, edgeql, hash, name, parseResult);
        }
        ...
        private static GenerationResult GenerateCSharpFromEdgeQL(string @namespace, string edgeql, string hash, string resultName, EdgeDBBinaryClient.ParseResult parseResult)
        {
            var codecType = GetTypeOfCodec(parseResult.OutCodec.Codec, $"{resultName} Result");

            // create the class writer

            var writer = new CodeWriter();
            writer.AppendLine("// AUTOGENERATED: DO NOT MODIFY");
            writer.AppendLine($"// edgeql:{hash}");
            ...
            var classScope = writer.BeginScope($"public static class {resultName}");

            writer.AppendLine($"public static readonly string Query = @\"{edgeql}\";");
            writer.AppendLine();
            ...
            writer.AppendLine();
            writer.AppendLine($"public static Task<{resultType}> {resultName}Async(this IEdgeDBQueryable client{(argParameters.Any() ? $", {string.Join(", ", argParameters)}" : "")}, CancellationToken token = default)");
